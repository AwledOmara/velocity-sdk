...
class Processor
{
	...
	public function __construct($applicationprofileid, $merchantprofileid, $workflowid, $isTestAccount, $identitytoken = null, $sessiontoken = null ) {
            $this->connection = Connection::instance(); // VelocityConnection class object store in private data member $connection. 
        ...
        $this->sessionToken = empty($sessiontoken) && !empty($identitytoken) ? $this->connection->signOn () : $sessiontoken;
	...
	*/
    ...
    public function verify($options = array()) {
            ...
            try { 
                    ...
                    $xml = XmlCreator::verifyXML($options);  // got Verify xml object.
	...
	public function authorizeAndCapture($options = array()) { 
            ...
            try {
                    ...
                    $xml = XmlCreator::authorizeandcaptureXML($options);  // got authorizeandcapture xml object.
                    $xml->formatOutput = TRUE;
                    $body = $xml->saveXML();
                    //echo '<xmp>'.$body.'</xmp>'; die;
                    list($error, $response) = $this->connection->post(
	...
	*/
	...
	public function authorize($options = array()) {
            ...
            try {
                    $xml = XmlCreator::authorizeXML($options); // got authorize xml object.
                    $xml->formatOutput = TRUE;
                    $body = $xml->saveXML();
                    //echo '<xmp>'.$body.'</xmp>'; 
                    list($error, $response) = $this->connection->post(
	...
	*/
	public function capture($options = array()) {
            ...
            if(isset($options['amount']) && isset($options['TransactionId'])) {
                    ...
                    try {
                            $xml = XmlCreator::captureXML($options['TransactionId'], $amount);  // got capture xml object.
                            $xml->formatOutput = TRUE;
                            $body = $xml->saveXML();
                            //echo '<xmp>'.$body.'</xmp>'; die;
                ...
                throw new Exception(Message::$descriptions['errcapsesswfltransid']);
            }
	}
	...
	*/
	public function captureAll($options = array()) {
            ...
            try {
                    $xml = XmlCreator::captureAllXML();  // got capture xml object.
                    $xml->formatOutput = TRUE;
                    $body = $xml->saveXML();
                    //echo '<xmp>'.$body.'</xmp>'; die;
                    list($error, $response) = $this->connection->put(
	...
	*/
	public function adjust($options = array()) {
            ...
            if( isset($options['amount']) && isset($options['TransactionId']) ) {
                    $amount = number_format($options['amount'], 2, '.', '');
                    try {
                            $xml = XmlCreator::adjustXML($options['TransactionId'], $amount);  // got adjust xml object.
                            $xml->formatOutput = TRUE;
                            $body = $xml->saveXML();
                            //echo '<xmp>'.$body.'</xmp>'; die;
                            list($error, $response) = $this->connection->put(
                    ...
                    throw new Exception(Message::$descriptions['erradjustsesswfltransid']);
            }
	}
	...
	public function undo($options = array()) {
            ...
            if ( isset($options['TransactionId']) ) {
                    ...
                    try {
                            $xml = XmlCreator::undoXML($options['TransactionId']);  // got undo xml object.
                            $xml->formatOutput = TRUE;
                            $body = $xml->saveXML();
                            list($error, $response) = $this->connection->put( 
                                                                            $this->path(
                    ...
                    throw new Exception(Message::$descriptions['errundosesswfltransid']);
            }
	}
	...
	public function returnById($options = array()) {
            ...
            if(isset($options['amount']) && isset($options['TransactionId'])) {
                    $amount = number_format($options['amount'], 2, '.', '');
                    try {
                            $xml = XmlCreator::returnByIdXML($amount, $options['TransactionId']);  // got ReturnById xml object.
                            $xml->formatOutput = TRUE;
                            $body = $xml->saveXML();
                            //echo '<xmp>'.$body.'</xmp>'; die;
                            list($error, $response) = $this->connection->post(
                    ...
                    throw new Exception(Message::$descriptions['errreturntranidwid']);
            }  
	}
	...
	public function returnUnlinked($options = array()) {
            ...
            try {
                    $xml = XmlCreator::returnUnlinkedXML($options);  // got ReturnById xml object.
                    $xml->formatOutput = TRUE;
                    $body = $xml->saveXML();
                    //echo '<xmp>'.$body.'</xmp>'; die;
                    list($error, $response) = $this->connection->post(
	...
	private function path($arg1, $arg2, $rtype) {
            if(isset($arg1) && isset($arg2) && isset($rtype) && ( $rtype == self::$Txn_method[3] || $rtype == self::$Txn_method[4] || $rtype == self::$Txn_method[5] || $rtype == self::$Txn_method[0] ) ) {
                    ...
                    throw new Exception(Message::$descriptions['errcapadjpath']);
            }
	}
